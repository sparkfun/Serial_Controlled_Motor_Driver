...
volatile uint8_t CONFIG_BITS = 0x01;
//0 -- UART
//1 -- SPI
...
const USER_PORT_SPI_INIT_STRUCT configSPI =
{
    ...
    USER_PORT_BITS_ORDER_LSB_FIRST, //USER_PORT_SPI_BITS_ORDER,
...
//    USER_PORT_Start();     /* Enabling the UART */
    ...
    if(CONFIG_BITS == 1) //SPI
    {
        ...
        while(1)
        {
            ...
            {
                uint8_t rxTemp;
                while(USER_PORT_SpiUartGetRxBufferSize())
                {
                    rxTemp = USER_PORT_SpiUartReadRxData();
                }
                USER_PORT_SpiUartClearRxBuffer();
        ...
        while(1)
        {
            ...
            if (0u != (USER_PORT_I2CSlaveStatus() & USER_PORT_I2C_SSTAT_WR_CMPLT))
            {
                ...
                if (USER_PORT_I2CSlaveGetWriteBufSize() == 2)
                {
                    ...
                    if((bufferRx[0] >= 2) && (bufferRx[0] < 0x20))
                    {
                        addressPointer = bufferRx[0];
                        registerTable[addressPointer] = bufferRx[1];
                    }
            ...
            bufferTx[0] = registerTable[addressPointer];
